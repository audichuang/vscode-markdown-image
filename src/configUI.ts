...
export async function configureSettings(): Promise<void> {
    ...
    try {
        await config.update('path', selectedPath, configTarget);
        await config.update('basePath', selectedBasePath, configTarget);
        ...
        if (showSettings === '查看設定') {
            if (isWorkspace) {
                ...
                // Open user settings and search for pasteImage
                await vscode.commands.executeCommand(
...
export async function showCurrentSettings(): Promise<void> {
    ...
    const path = config.get<string>('path') || '${currentFileDir}';
    const basePath = config.get<string>('basePath') || '${currentFileDir}';
    const defaultName = config.get<string>('defaultName') || 'YYYY-MM-DD-HH-mm-ss';
