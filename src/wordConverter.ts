...
export async function convertWordToMarkdown(): Promise<void> {
    ...
    const wordFilePath = fileUri[0].fsPath;
    // 處理大小寫不同的副檔名
    ...
    if (fs.existsSync(mdFilePath)) {
        const overwrite = await vscode.window.showWarningMessage(
            `File ${wordFileName}.md already exists. Overwrite?`,
            'Yes', 'No'
        );
        if (overwrite !== 'Yes') {
            return;
    ...
    await vscode.window.withProgress({
        ...
        try {
            progress.report({ increment: 10, message: 'Reading Word document...' });
            ...
            progress.report({ increment: 20, message: 'Converting content and extracting images...' });

            // 直接在轉換過程中處理圖片，確保順序正確
            const result = await convertDocxWithImages(wordFilePath, imagesDir, wordFileName);

            progress.report({ increment: 50, message: 'Saving Markdown file...' });
            ...
            showInformationMessage(
                `Conversion complete! Markdown: ${wordFileName}.md, Images: ${result.imageCount} extracted`
            );

            log(`Converted ${wordFilePath} to ${mdFilePath}, extracted ${result.imageCount} images`);

        } catch (err) {
            showErrorMessage(`Conversion failed: ${(err as Error).message}`);
...
export function sanitizeFileName(name: string): string {
    ...
    let result = name
        .replace(/[<>:"/\\|?*\x00-\x1f\x7f]/g, '_')  // 危險字元 + DEL
        .replace(/\.{2,}/g, '_')                      // 路徑穿越
        .replace(/^\.+/, '_')                         // 開頭的點
        .replace(/[\s.]+$/, '_')                      // 結尾空格和點
        .slice(0, 200);
